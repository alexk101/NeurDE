...
class AdaptiveGradientClipper:
    ...
    def __init__(self, cfg):
        ...
        self.clip_option = (
            cfg.get("clip_option", "adaptive_scaling").lower()
            if cfg.get("clip_option", None)
            else None
        )
    ...
    def _initialize_ema(self):
        ...
        self.var = max(
            sum((x - self.mean) ** 2 for x in valid_buffer) / len(valid_buffer),
            self.eps,
        )
        self.initialized = True
        self.buffer = []
    ...
    def _update_ema(self, grad_norm):
        ...
        self.var = (
            self.alpha * self.var + (1 - self.alpha) * (grad_norm - old_mean) ** 2
        )
    ...
    def step(self, trainer, model):
        ...
        if not self.enabled:
            ...
            if self.max_grad_norm is not None:
                ...
                if total_norm_float > self.max_grad_norm:
                    ...
                    trainer.log.info(
                        f"Static clipping: norm={float(total_norm):.4f}, threshold={self.max_grad_norm}"
                    )
                return total_norm
            else:
        ...
        if self.max_grad_norm is not None:
            if total_norm_float > self.max_grad_norm:
                ...
                self.clips_per_log_interval += 1
                trainer.log.info(
                    f"Max norm clipping: norm={float(total_norm):.4f}, threshold={self.max_grad_norm}"
                )
                return total_norm
        ...
        if clip_val is not None:
            ...
            self.clips_per_log_interval += 1
            trainer.log.info(
                f"Adaptive clipping: norm={float(total_norm):.4f}, z_score={z_score:.2f}, "
        ...
        if not (clip_val is not None and self.skip_update_on_spike):
            ...
            if math.isfinite(ema_update_value):
                ...
                trainer.log.warning(
                    f"Skipping EMA update with non-finite value: {ema_update_value}"
                )
    ...
    def get_stats(self):
        ...
        return {
            ...
            + (self.warmup_steps if self.initialized else 0),
    ...
    def load_state_dict(self, state_dict, strict=True):
                ...
                if key in state_dict:
                    ...
                    if current_val != saved_val:
                        config_mismatches.append(
                            f"{key}: current={current_val}, saved={saved_val}"
                        )
        ...
        # Load runtime state
        self.buffer = (
            state_dict["buffer"].copy()
            if isinstance(state_dict["buffer"], list)
            else list(state_dict["buffer"])
        )
